...
SliderButton::SliderButton(const std::string& sliderBarFileName, const std::string& sliderPinFilename)
{
	//TODO FIX TAG AND BE ABLE TO CREATE MORE THEN ONCE SLIDER
	SetTag(sliderBarFileName);
	m_mousePosition = { 0, 0, 0, 0 };
	m_sliderState = SliderState::Default;
	//Sprites
	m_sliderBar.Load("Assets/Buttons/" + sliderBarFileName + "Button.png");
	m_sliderBar.SetImageDimension(1, 1, 206, 17);
	m_sliderBar.SetSpriteDimension(207, 17);
	
	m_sliderPin.Load("Assets/Buttons/" + sliderPinFilename + "Button.png");
...
void SliderButton::SetSliderPinPosition()
{
	...
	{
		if (this->GetPinPosition().x >= m_sliderPosition.x - 5 && this->GetPinPosition().x <= m_sliderPosition.x + m_sliderPosition.w)
		{
			m_pinPosition.x = Input::Instance()->GetMousePosition().x - 5;
		}
	}
}
